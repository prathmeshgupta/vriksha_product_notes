...
**`products.regulatory_regime`** (Phase 1, new column on the existing `products` table)
Text column, e.g. `india_sebi`, `other`, `none` (extend as real non-India regimes get defined). Drives which compliance checks apply to that product — see §2.3. Not a new table; a single addition to `products` alongside `code`, `category`, etc.
...
### 2.3 Compliance rules are scoped by regime, not universal
...
- `india_sebi`: single-stock cap, sector cap, risk-profile-to-allocation consistency, SEBI disclosure checklist — plus the universal weights-sum-to-100% check.
- other regimes: start with just the universal weights check until that regime's specific rules are defined; not blocked on every jurisdiction being fully specified before Phase 1 can ship.

This mapping (regime → which checks apply, and their thresholds) lives in application logic for now, reading limits from each product's existing note fields (e.g. `data.portfolioConstructionRules.singleStockCap`) rather than being hardcoded or duplicated into a separate rules table. Revisit as a proper `compliance_rules` table only if the mapping outgrows what's comfortable to keep in code.
...
- **Casual edits vs. audit-grade actions**: publishing a product note is single-user, immediate. Publishing a *rebalance* is also single-user/immediate for now (automated checks only, no maker-checker — deferred as of 2026-07-16 to avoid standing up a roles system for one workflow) — but it still writes through the same append-only `_history` pattern so the audit trail exists regardless of whether a second approver is required later.
