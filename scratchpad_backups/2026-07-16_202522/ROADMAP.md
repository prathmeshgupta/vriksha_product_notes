...
**Goal**: automated checks before any rebalance/portfolio change is published, plus a maker-checker approval step.
...
**Checks to automate** (pull thresholds from each product's existing note fields — e.g. `portfolioConstructionRules.singleStockCap`, `.sectorCap` — so the engine reads limits from the note rather than hardcoding them):
...
- Weights sum to 100% (± rounding tolerance)
- SEBI disclosure requirement checklist (static checklist item, human-confirmed — not all of this is mechanically checkable)

**Workflow**: maker (anyone) proposes a holdings change → engine runs checks automatically → pass/fail + details shown inline → if pass, a checker (different logged-in user) approves → only then does it write to `portfolio_holdings` and append to `portfolio_holdings_history` with `approved_by` set. If fail, the maker sees exactly which rule broke and by how much.

**UI**: new "Compliance" nav item. Shows: pending approvals queue, check results per pending change, a log of past checks (pass and fail, not just approved ones — useful for audit).

**Open questions to resolve before building** (ask user):
- Does "checker" need to be a specific named role, or just "any second logged-in user, not the same person who proposed it"? (Affects whether we need a roles table or can do it with a simple `!=` check on user id.)
- Should a failed check block publishing entirely, or allow an override with a logged reason (e.g. "breach approved by CIO, see email")?
