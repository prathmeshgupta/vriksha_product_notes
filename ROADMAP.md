...
Last updated: 2026-07-16. Sequenced per user priority: compliance engine and client-facing reporting first (these two are the path to actually sending product notes to clients), then tax module, then constituent UI, then live pricing/performance, then smallcase integration. Two ad-hoc requests (PDF export, Create/Templates redesign) were pulled forward ahead of Phase 1 — see Phase 0.5 below — because they were flagged as existing-functionality gaps, not new scope.
...
| 0.5 | PDF export + Create/Templates redesign (ad-hoc, pulled forward) | ✅ Done |
| 1 | Compliance engine | ✅ Done |
...
## Phase 0.5 — PDF Export + Create/Templates Redesign (ad-hoc, shipped 2026-07-16)

Two gaps in existing functionality, raised mid-build and shipped ahead of Phase 1 since they were quick, self-contained, and didn't block on any new schema beyond one new table.

**PDF export**: added `exportPdf(pid)` (full note, mirrors the Word export's structure) and `exportClientSummaryPdf(pid)` (fixed short field set — objective, risk profile, benchmark, minimum investment, suitability, top 5 key risks — genuinely 1-2 pages) using jsPDF + jsPDF-autotable. Both use a light, print-appropriate theme derived from the app's dark web palette (`PDF_THEME`). Buttons added alongside the existing Word/Excel export buttons on every product page.

**Create/Templates redesign**: replaced the old single "Template" dropdown (which forced choosing from the 9 real products, framed as templates) with three explicit paths on the Create screen — Start Blank, From Archetype Template, From Existing Product. Added a new `templates` Supabase table, deliberately separate from `products` so normal product edit/archive/delete code paths never touch template rows (see ARCHITECTURE.md §2.3). Seeded with 4 starter archetypes: Single-Sleeve Equity, Strategic Allocation, Risk-Profiled Variants, Goal-Based. Added a full "Manage Templates" screen (new nav item) with create/edit/delete CRUD, kept entirely separate from the product Edit screen.

Both shipped in commit `011141d`, confirmed live.
